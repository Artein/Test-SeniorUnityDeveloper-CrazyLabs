...
{
    private static readonly Vector3 TestOrigin = new(x: 1100f, y: 0f, z: 1100f);
    ...
    [TestCase(arg1: 0.01f, arg2: 6, arg3: 4)]
    [TestCase(arg1: 0.02f, arg2: 3, arg3: 2)]
        ...
        int discontinuityTick)
    {
        ...
        var acquired = fixture.Tick(label: "flat acquire");
        ...
        Assert.That(acquired.DiagnosticsText, Does.Contain(expected: "observed:Supported normal:"), acquired.FailureContext);
        Assert.That(acquired.DiagnosticsText, Does.Contain(expected: "stable:grounded normal:"), acquired.FailureContext);
        Assert.That(acquired.DiagnosticsText, Does.Contain(expected: "transition:SupportAcquired"), acquired.FailureContext);
        Assert.That(acquired.DiagnosticsText, Does.Contain(expected: "steering:valid up:"), acquired.FailureContext);

        fixture.ShowNone();
        var briefGap = fixture.Tick(label: "brief gap");

        Assert.That(
            briefGap.Snapshot.ObservedSupport.State,
            Is.EqualTo(RunSupportObservationState.Missing),
            briefGap.FailureContext);
        ...
        Assert.That(briefGap.DiagnosticsText, Does.Contain(expected: "observed:Missing normal:n/a"), briefGap.FailureContext);
        Assert.That(briefGap.DiagnosticsText, Does.Contain(expected: "stable:grounded normal:"), briefGap.FailureContext);
        Assert.That(briefGap.DiagnosticsText, Does.Contain(expected: "held:yes"), briefGap.FailureContext);

        fixture.ShowFlat();
        var recoveredGap = fixture.Tick(label: "brief gap recovery");
        ...
        for (var missTick = 1; missTick < supportLossTick; missTick += 1)
        {
            ...
            Assert.That(
                held.Snapshot.ObservedSupport.State,
                Is.EqualTo(RunSupportObservationState.Missing),
                held.FailureContext);
        ...
        Assert.That(
            lost.Snapshot.ObservedSupport.State,
            Is.EqualTo(RunSupportObservationState.Missing),
            lost.FailureContext);
        ...
        Assert.That(lost.DiagnosticsText, Does.Contain(expected: "stable:unsupported normal:n/a"), lost.FailureContext);
        Assert.That(lost.DiagnosticsText, Does.Contain(expected: "transition:SupportLost"), lost.FailureContext);

        fixture.ShowFlat();
        var reacquired = fixture.Tick(label: "gap reacquire");
        ...
        for (var seamTick = 1; seamTick < discontinuityTick; seamTick += 1)
        {
            ...
            Assert.That(
                confirming.Snapshot.ObservedSupport.State,
                Is.EqualTo(RunSupportObservationState.Supported),
            ...
            Assert.That(confirming.DiagnosticsText, Does.Contain(expected: "confirming:yes"), confirming.FailureContext);
        }
        ...
        Assert.That(
            confirmed.DiagnosticsText,
            Does.Contain(expected: "transition:ConfirmedDiscontinuity"),
            confirmed.FailureContext);

        Assert.That(confirmed.DiagnosticsText, Does.Contain(expected: "confirming:no"), confirmed.FailureContext);

        Assert.That(
            Vector3.Angle(confirmed.Snapshot.SteeringFrame.UpDirection, fixture.BankNormal),
            Is.LessThanOrEqualTo(expected: 2f),
            confirmed.FailureContext);
    }
        ...
        int discontinuityTick)
    {
        var fixture = CreateFixture(fixedDeltaTime);
        fixture.ShowFlat();
        AssertSupported(fixture.Tick(label: "flat acquire"), RunSurfaceTransition.SupportAcquired, Vector3.up);
        ...
        for (var alternatingTick = 1; alternatingTick <= 6; alternatingTick += 1)
        {
            ...
            Assert.That(
                sample.Snapshot.ObservedSupport.State,
                Is.EqualTo(RunSupportObservationState.Supported),
    ...
    public void given_ContinuousSlopeAndUnavailableFrame_when_PipelineTicks_then_UpdatesAndHardResetsExactlyOnce()
    {
        ...
        var continuous = fixture.Tick(label: "continuous slope");

        AssertSupported(
            continuous,
            RunSurfaceTransition.ContinuousUpdate,
            fixture.GentleSlopeNormal,
            toleranceDegrees: 2f);
        ...
        Assert.That(
            Vector3.Angle(Vector3.up, steeringUp),
            Is.GreaterThan(expected: 0f).And.LessThanOrEqualTo(expected: 3.7f),
            continuous.FailureContext);

        Assert.That(
            Vector3.Angle(steeringUp, fixture.GentleSlopeNormal),
            Is.LessThan(expected: 20f),
            continuous.FailureContext);

        fixture.IsProgressFrameAvailable = false;
        var hardReset = fixture.Tick(label: "unavailable frame");

        Assert.That(
            hardReset.Snapshot.ObservedSupport.State,
            Is.EqualTo(RunSupportObservationState.Unavailable),
        ...
        Assert.That(hardReset.DiagnosticsText, Does.Contain(expected: "observed:Unavailable normal:n/a"), hardReset.FailureContext);
        Assert.That(hardReset.DiagnosticsText, Does.Contain(expected: "transition:HardReset"), hardReset.FailureContext);
        Assert.That(hardReset.DiagnosticsText, Does.Contain(expected: "steering:unavailable up:n/a"), hardReset.FailureContext);

        var repeatedUnavailable = fixture.Tick(label: "repeated unavailable frame");

        Assert.That(
            repeatedUnavailable.Snapshot.Transition,
            Is.EqualTo(RunSurfaceTransition.HardReset),
    ...
    {
        var body = CreateGameObject(name: "Run Surface Trace Body");
        ...
        var flat = CreateSurface(name: "Flat", Quaternion.identity);
        var gentleSlope = CreateSurface(name: "Gentle Slope", Quaternion.AngleAxis(angle: 20f, Vector3.forward));
        var bank = CreateSurface(name: "Bank", Quaternion.AngleAxis(angle: 60f, Vector3.forward));
        var oppositeBank = CreateSurface(name: "Opposite Bank", Quaternion.AngleAxis(angle: -60f, Vector3.forward));
    ...
    private GameObject CreateSurface(string name, Quaternion rotation)
    {
        ...
        var size = new Vector3(x: 4f, y: 0.1f, z: 4f);
        ...
        float toleranceDegrees = 0.1f)
    {
        ...
        Assert.That(sample.Snapshot.ObservedSupport.SupportDistance, Is.GreaterThan(expected: 0f), sample.FailureContext);
        Assert.That(sample.Snapshot.StableSupport.IsGrounded, Is.True, sample.FailureContext);

        Assert.That(
            Vector3.Angle(sample.Snapshot.StableSupport.GroundNormal, expectedNormal),
    ...
    private readonly struct RunSurfaceTraceSample
    {
        ...
        public RunSurfaceTraceSample(int tick, string label, RunSurfaceFrameSnapshot snapshot)
        {
            ...
            DiagnosticsText = RunDiagnosticsOverlayTextFormatter.FormatMotionSummary(diagnosticsSample);
