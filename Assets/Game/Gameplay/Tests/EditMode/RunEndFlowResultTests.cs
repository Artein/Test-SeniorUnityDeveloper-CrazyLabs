...
public sealed partial class RunEndFlowTests
{
    ...
    {
        ActivateRun();
        LogAssert.Expect(LogType.Log, new Regex("Run Result: Reason=Finished, IsSuccess=True, ElapsedTime=0, DistanceTravelled=12.5"));
    ...
    public void FixedTick_InvalidProgressSnapshot_NotifiesDegradedRunResultAndCanAcknowledge()
    {
        ...
        LogAssert.Expect(LogType.Log, new Regex("Run Result: Reason=Finished, IsSuccess=True, ElapsedTime=0, DistanceTravelled=0"));
        ...
        Assert.That(acceptedResults[0].Reason, Is.EqualTo(RunEndReason.Finished));
        Assert.That(acceptedResults[0].IsSuccess, Is.True);
        Assert.That(acceptedResults[0].ElapsedTime, Is.Zero.Within(0.001f));
