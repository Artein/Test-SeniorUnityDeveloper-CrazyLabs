...
namespace Game.Gameplay.CharacterPresentation
{
    internal sealed class CharacterPresentationPresenter : IInitializable, ITickable, IDisposable
    {
        ...
        void ITickable.Tick()
        {
            ...
            var surfaceContext = isPreLaunch ? new RunSurfaceContext(true, Vector3.up, 0f) : _surfaceContextSource.Current;
            ...
            var input = new CharacterPresentationClassificationInput(_currentMode, _currentModeElapsedSeconds, _ungroundedElapsedSeconds, isPreLaunch,
                isRunActive, _hasAcceptedRunResult, _acceptedRunResultSucceeded, surfaceContext, coursePlanarSpeed, courseForwardSpeed,
                linearVelocity);

            var result = _classifier.Classify(input);
            UpdateModeElapsed(result.Mode, deltaTime);
            _view.ApplyFrame(new CharacterPresentationFrame(result.Mode, CalculatePlaybackSpeed(result.Mode, coursePlanarSpeed)));
        }
