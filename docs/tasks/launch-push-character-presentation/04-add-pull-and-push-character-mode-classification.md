...
The classifier should keep terminal modes highest priority, then select Pull Anticipation while a validated active pull exists, then select Launch Push while a launch-push fact is active and its elapsed time is below the configured minimum guard. Existing pre-launch, Slide, reserved Run compatibility, Airborne, Victory, Defeat, speed, hysteresis, and airborne behavior should continue to apply after slingshot-specific modes no longer own presentation.
...
- [ ] LaunchPush outranks Idle, Slide, reserved Run compatibility, and Airborne while launch push is active and elapsed time is below the minimum guard.
...
- [ ] Existing Slide, reserved Run compatibility, Airborne, terminal, hysteresis, and minimum locomotion mode duration behavior remains covered and unchanged outside the new precedence rules.
...
- EditMode tests:
  ...
  - PullAnticipation is selected over flat grounded locomotion facts.
  - LaunchPush is selected over downhill Slide before the minimum guard elapses.
  - LaunchPush is selected over flat grounded locomotion before the minimum guard elapses.
  ...
  - Existing classifier tests for Slide, reserved Run compatibility, Airborne, hysteresis, and terminal behavior remain green.
- PlayMode tests:
